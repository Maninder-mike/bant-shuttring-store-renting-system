...
sip API incompatibility issue in bssrs's non-gui modules)
"""
...
# from bssrs.utils import encoding
# from bssrs.py3compat import is_unicode, PY3, to_text_string, is_text_string
...
def get_conf_subfolder():
    ...
    # since 3.0+ we've reverted back to use .bssrs to simplify major
...
def get_clean_conf_dir():
    ...
    conf_dir = osp.join(str(tempfile.gettempdir()),
                        'pytest-bssrs{0!s}'.format(current_user),
                        get_conf_subfolder())
    return conf_dir
...
def get_conf_path(filename=None):
    ...
    if running_under_pytest() or SAFE_MODE:
        ...
        # on Linux, as it was requested on bssrs-ide/bssrs#2629.
...
# DEFAULT_LANGUAGE = 'en'
#
# # This needs to be updated every time a new language is added to bssrs, and is
...
#     """
#     List available translations for bssrs based on the folders found in the
...
#     """
#     locale_path = get_module_data_path("bssrs", relpath="locale",
